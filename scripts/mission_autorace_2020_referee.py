...
class ControlMission():
    def __init__(self):
        ...
        self.sub_robot_finish = rospy.Subscriber("robot_finish", String, self.callback_robot_finish)
        self.rate = rospy.Rate(5)
        self.initial_pose = Pose()
        self.traffic_state = 1
        self.time_robot_start = rospy.Duration(0)
        self.time_robot_finish = rospy.Duration(0)
        ...
        while not rospy.is_shutdown():
            if self.traffic_state == 1:  # turn on red light
                ...
                if abs(self.current_time - time.time()) > 5:  # turn on green light after 5s.
                    ...
                    self.time_robot_start = rospy.get_time()
                    rospy.loginfo("Robot start time %f", self.time_robot_start)
